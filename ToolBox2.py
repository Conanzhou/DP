...
class complexSignal():
    ...
    def plot_power(self, start=None, end=None,xlim=(-800,800),isPlotFlip=False,isPlotDelta=False,label=None):
        """Extracts a segment.
...
class Signal(complexSignal):
    def __init__(self, shotNum, channelList, ts_s=100, ts_e=1500, step=1, Device = 'HL-2M'):
        ...
        self.Device = Device
        self.shotNum = shotNum
        self.chl = channelList

        if len(self.chl)==2:
            ts1, y1, U = hldb(shotNum,self.chl[0], ts_s/1e3, ts_e/1e3,step)
            ts2, y2, U = hldb(shotNum,self.chl[1], ts_s/1e3, ts_e/1e3,step)
            self.ys = np.asanyarray(y1+y2*1J)
        elif len(self.chl)==1:
            ts1, y1, U = hldb(shotNum,self.chl, ts_s/1e3, ts_e/1e3,step)
            self.ys = np.asanyarray(y1)
        else:
            print('check the channel input')
