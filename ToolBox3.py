...
class complexSignal():
    ...
    def plot(self,isDecorate=False, **options):
        """Plots the wave.
    ...
    def spectrogram(self,para=[512*2,350*2,1024*2],isPlot=False,isNormalize=False,isDecorate=True,ylim=(-500,500),window='hamming',cmap='jet',mode='psd',scaling='density',**options):
        """
        ...
        if isPlot is True:
            if isNormalize:
                sum_S = np.sum(self.spect_Sxx,axis=0)
            else:
                sum_S = 1
            im = plt.pcolormesh(self.spect_t, self.spect_f, 10*np.log10(self.spect_Sxx/sum_S), cmap=cmap, shading='auto',**options)
            plt.ylim(ylim)
            return im
    ...
    def plot_power(self, start=None, end=None,xlim=(-800,800),isDecorate=True,isPlotFlip=False,isPlotDelta=False,label=None):
        """Extracts a segment.
        ...
        p = plt.plot(f, 10*np.log10(Pxx),label=label)
        if isPlotFlip:
    ...
    def gaussian3(self,data=None,para=[2e-8,-200,1,6e-8,200,1],isLog=True):
        if data is None:
            ...
            x = data[0]
            y = data[1]
        gmodel = GaussianModel(prefix='p1_') + GaussianModel(prefix='p2_')+GaussianModel(prefix='p3_')
