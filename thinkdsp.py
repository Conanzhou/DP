...
class _SpectrumParent:
    ...
    def plot_power(self, high=None, **options):
        ...
        if self.full:
            fs, amps = self.render_full(high)
            plt.plot(fs, 10*np.log10(amps ** 2), **options)
            # plt.yscale('log')
        else:
            i = None if high is None else find_index(high, self.fs)
...
class Spectrogram:
    ...
    def plot(self, high=None, **options):
        ...
        for j, t in enumerate(ts):
            ...
            array[:, j] = spectrum.power[:i] / (len(fs)*self.freq_res)

        # underride(options, cmap='inferno_r', shading='auto')
        underride(options, cmap='jet')
        # plt.pcolormesh(ts, fs, array, **options)
        plt.pcolormesh(ts, np.fft.fftshift(fs), 10*np.log10(np.fft.fftshift(array,axes=0)), **options)
...
class Wave:
    ...
    def make_spectrogram(self, seg_length, win_flag=True,full=False):
        ...
        while j < len(self.ys):
            ...
            spec_map[t] = segment.make_spectrum(full=full)
    ...
    def plot(self, **options):
        ...
        plt.plot(self.ts * xfactor, np.imag(self.ys), **options)
        # else:
            # plt.plot(self.ts * xfactor, np.real(self.ys), **options)
